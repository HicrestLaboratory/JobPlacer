...
impl fmt::Display for TopologyIR {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if !cells.is_empty() {
            ...
            for (cell_name, stats) in &cells {
                let partition_str = if stats.partitions.is_empty() {
                    ...
                    stats.partitions.iter().cloned().collect::<Vec<_>>().join(", ")
                };
                writeln!(
                    f,
        ...
        let mut roots: Vec<&Id> = self.contains.keys()
...
fn compress_ids(ids: &[&Id]) -> String {
    ...
    let parsed: Vec<(&str, Option<u64>)> = ids.iter().map(|id| {
        let s = id.0.as_str();
    ...
    if !all_same_prefix {
        ...
        return ids.iter().map(|id| id.0.as_str()).collect::<Vec<_>>().join(", ");
    }
...
fn fmt_subtree(
    ...
    let kind_tag = match entities.get(id).map(|e| &e.kind) {
        Some(EntityKind::Switch { level: Some(0) }) => {
            let cell = entities.get(id)
                .and_then(|e| e.meta.get("cell"))
                .map(|s| s.as_str())
                .unwrap_or("?");
            let rack = entities.get(id)
    ...
    if depth >= max_depth {
        if let Some(children) = contains.get(id) {
            if !children.is_empty() {
                writeln!(f, "{}  … ({} children, depth limit)", indent, children.len())?;
            }
        }
        return Ok(());
    }
    ...
    if !l2_switch_ids.is_empty() {
        writeln!(f, "{child_indent}[L2 switches] {}", compress_ids(&l2_switch_ids))?;
    }
    if !l1_switch_ids.is_empty() {
        writeln!(f, "{child_indent}[L1 switches] {}", compress_ids(&l1_switch_ids))?;
    }
