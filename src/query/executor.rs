use super::{Constraint, NodePredicate, QueryError, ReferencePoint};
...
impl TopologyQuery {
    ...
    pub fn execute_from(&self, ir: &TopologyIR, anchor: Id) -> Result<Vec<Id>, QueryError> {
        // Filter to only compute nodes
        let compute_ir = ir.filter(|entity| matches!(entity.kind, EntityKind::Compute));
...
impl<'a> NodeSelector<'a> {
    ...
    fn resolve_reference(&mut self, reference: &ReferencePoint) -> Result<Id, QueryError> {
        match reference {
            ...
            ReferencePoint::NodeId(id) => Ok(id.clone()),

            ReferencePoint::AnyMatching(predicate) => self
                .available
                .iter()
                .find(|id| {
    ...
    fn find_nodes_at_distance_with_shared_parent(
        ...
        for (_, nodes) in &groups_by_parent {
            // ← Changed: borrowed instead of moved
            if nodes.len() >= count {
        ...
        let max_available = groups_by_parent
            .values()
            .map(|v| v.len())
            .max()
            .unwrap_or(0);
...
fn matches_predicate(entity: &Entity, predicate: &NodePredicate) -> bool {
    match predicate {
        NodePredicate::NodeKind(kind) => match &entity.kind {
        ...
        NodePredicate::IdPattern(pattern) => entity.id.0.contains(pattern),
