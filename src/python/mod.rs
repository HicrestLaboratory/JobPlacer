...
fn format_nodelist_simple(nodes: &[Id]) -> String {
    nodes.iter().map(|id| id.0.as_str()).collect::<Vec<_>>().join(",")
}
...
fn format_nodelist_compressed(nodes: &[Id]) -> String {
    ...
    let node_strings: Vec<&str> = nodes.iter().map(|id| id.0.as_str()).collect();
    try_compress_nodes(&node_strings).unwrap_or_else(|| format_nodelist_simple(nodes))
}
...
/// Split node name into prefix and numeric suffix
fn split_prefix_number(s: &str) -> Option<(String, usize)> {
    ...
    if num_start == s.len() { return None; }
...
fn build_ranges(numbers: &[usize]) -> String {
    if numbers.is_empty() { return String::new(); }
    let mut ranges = Vec::new();
    let mut start = numbers[0];
    let mut end = numbers[0];
    for &num in &numbers[1..] {
        if num == end + 1 { end = num; } 
        else { ranges.push(format_range(start, end)); start = num; end = num; }
    }
...
/// Python class for TopologyQueryBuilder
#[pyclass]
...
impl TopologyQueryBuilder {
    ...
    fn get_nodelist_distances(&self, anchor: String, distances: Vec<(usize, f32)>) -> PyResult<String> {
        let anchor_id = Id::from(anchor.as_str());
        let groups: Vec<DistanceGroup> = distances.into_iter().map(|(count, distance)| DistanceGroup { count, distance }).collect();
        let query = TopologyQuery::new().with_constraint(Constraint::DistanceGroup { reference: ReferencePoint::First, groups });
        let selected = query.execute_from(&self.ir, anchor_id).map_err(|e| PyRuntimeError::new_err(format!("{}", e)))?;
        Ok(format_nodelist_compressed(&selected))
    }

    fn get_nodelist_distances_shared_parent(&self, anchor: String, distances: Vec<(usize, f32, usize)>) -> PyResult<String> {
        let anchor_id = Id::from(anchor.as_str());
        let groups: Vec<DistanceGroupWithParent> = distances.into_iter().map(|(count, distance, parent_level)| DistanceGroupWithParent { count, distance, parent_level }).collect();
        let query = TopologyQuery::new().with_constraint(Constraint::DistanceGroupWithSharedParent { reference: ReferencePoint::First, groups });
        let selected = query.execute_from(&self.ir, anchor_id).map_err(|e| PyRuntimeError::new_err(format!("{}", e)))?;
        Ok(format_nodelist_compressed(&selected))
    }
    ...
    fn get_nodelist_different_distances(&self, anchor: String, total_nodes: usize) -> PyResult<String> {
        ...
        while remaining > 0 {
            let count = remaining.min(2);
            groups.push(DistanceGroup { count, distance: pattern[pattern_idx % pattern.len()] });
            remaining -= count;
            pattern_idx += 1;
        }
        let query = TopologyQuery::new().with_constraint(Constraint::DistanceGroup { reference: ReferencePoint::First, groups });
    ...
    fn get_compute_nodes(&self) -> Vec<String> {
        ...
        self.ir.entities.iter().filter(|(_, e)| matches!(e.kind, EntityKind::Compute)).map(|(id, _)| id.0.clone()).collect()
    }

    fn is_valid_compute_node(&self, node_id: String) -> bool {
        use crate::ir::entity::EntityKind;
        let id = Id::from(node_id.as_str());
        self.ir.entities.get(&id).map(|e| matches!(e.kind, EntityKind::Compute)).unwrap_or(false)
    }
}

// #[pymodule]
// fn job_placer(_py: pyo3::prelude::Python, m: &pyo3::prelude::PyModule) -> pyo3::prelude::PyResult<()> {
